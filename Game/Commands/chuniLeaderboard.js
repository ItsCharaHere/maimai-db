...
const { SearchArgs, Constants, Commands, GameVersion, Locale, Ranks, CommandChuni } = require("./../constants.js");
...
async function chuniLeaderboard(game, msg, increment = 0, cache = null){
	let userParams = null;
	if (cache == null){
...
async function displayMythosLeaderboardsAsync(game, msg, cache, userParams, increment = 0){
	...
	proto = handleApiCalls("chunithm","leaderboard")

	try {
	var client = new proto.ChunithmLeaderboard(Secrets.MYTHOS,grpc.credentials.createSsl());
	} catch {
	...
	client.GetRating({version: 150}, requestMetadata, async function(err, response) {
		try {
	...
	if (cache == null){
		...
		cache.command = CommandChuni.LEADERBOARD;
		cache.page = 0;
	...
	let userCount = 0;
		...
		for (var i = 0; i < Constants.DefaultPageSize; i++)
		{
			...
			let ratingLabel = getRatingLabel(rating_cur, "chunithm");
		...
		let pre_description = `Viewing the current Mythos leaderboards. Type \`${Constants.PrefixChuni}stats <@user>\` for a more detailed summary of that user. (UNFINISHED)\n`;
		...
		if (userParams.version.id == game.game_version){
			pre_description += `- Version: \`Chunithm Paradise Lost (Mythos)\`\n`;
		} else {
...
module.exports = chuniLeaderboard;
